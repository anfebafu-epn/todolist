...
namespace ui {
    export class Task {
        ...
        static CurrentPersonID: nMorph.framework.common.datatypes.UUID;
        static CurrentCategoryID: nMorph.framework.common.datatypes.UUID;
        static LoggedUser: nMorph.core.db.sw_todolist.Person;
        ...
        static Init() {
            ...
            head.load("config.js?v=" + new Date().getTime().toString(), // Hace que este archivo nunca haga caché, siempre lea la última configuración
                function () {
                    head.load(
                        "lib/jquery/jquery-3.7.0.min.js", //"https://code.jquery.com/jquery-3.7.0.min.js",
                        ...
                        "assets/css/main.min.css?v=" + current_version,
                        "assets/js/web.js?v=" + current_version,
                        "assets/js/Base.js?v=" + current_version,
                        "assets/js/proxy.js?v=" + current_version,
                        "task.css?v=" + current_version,
                        "task.js?v=" + current_version,
                        function () {
                            Task.Load();
                        });
                }
        ...
        static Load() {
            ...
            Task.LoggedUser = <nMorph.core.db.sw_todolist.Person>nMorph.core.proxy.tools.Packager.Unserialize(psession);
            ...
            if (Task.LoggedUser.Role == 2) {
                ...
                ui.assets.js.Proxy.Person_Search("", (n: Array<nMorph.core.db.sw_todolist.Person>) => {
                    n.forEach((value: nMorph.core.db.sw_todolist.Person) => {
            ...
            ui.assets.js.Proxy.Category_Search("", (n: Array<nMorph.core.db.sw_todolist.Category>) => {
                n.forEach((value: nMorph.core.db.sw_todolist.Category) => {
            ...
            $(document).on('click', '.editButton', function () {
                ...
                const task = Task.getTaskById(taskId, (n: nMorph.core.db.sw_todolist.Task) => {
                    Task.openModal(n);
                });
            ...
            $('#btn_Save').click(function (event) {
                ...
                var taskData: nMorph.core.db.sw_todolist.Task = new nMorph.core.db.sw_todolist.Task();
        ...
        static showTasks(tasks: any[]) {
            const TasksContainer = $('#tasksContainer');
            TasksContainer.empty();

            tasks.forEach(task => {
                var p = <nMorph.core.db.sw_todolist.Task_VTA>task;
        ...
        static openModal(task: nMorph.core.db.sw_todolist.Task) {
            const modal = $('#editModal');
        ...
        static fetchTasks() {
            ...
            ui.assets.js.Proxy.Task_Search(searchTerm, Task.CurrentCategoryID, Task.CurrentPersonID, function (tasks: Task[]) {
                Task.showTasks(tasks);
            });
        }

        // Función para obtener los detalles de la taska por su ID
        static getTaskById(taskId: string, callback?: (n: nMorph.core.db.sw_todolist.Task) => void) {

            ui.assets.js.Proxy.Task_GetByID(nMorph.framework.common.datatypes.UUID.fromString(taskId),
                (n: nMorph.core.db.sw_todolist.Task) => {
                    if (callback) {
                        callback(n);
                    }
                });
