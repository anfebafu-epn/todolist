...
namespace ui {
    export class Category {
            ...
            head.load("config.js?v=" + new Date().getTime().toString(), // Hace que este archivo nunca haga caché, siempre lea la última configuración
                function () {
                    head.load(
                        ...
                        "category.css?v=" + current_version,
                        "category.js?v=" + current_version,
                        function () {
                            Category.Load();
                        });
                }
        ...
        static Load() {
            ...
            Category.LoggedUser = <nMorph.core.db.sw_todolist.Person>nMorph.core.proxy.tools.Packager.Unserialize(psession);
            ...
            $(document).on('click', '.editButton', function () {
                ...
                const category = Category.getCategoryById(categoryId, (n: nMorph.core.db.sw_todolist.Category) => {
            ...
            $('#btn_Save').click(function (event) {
                ...
                var categoryData: nMorph.core.db.sw_todolist.Category = new nMorph.core.db.sw_todolist.Category();
        ...
        static showCategories(categories: any[]) {
        ...
        static openModal(category: nMorph.core.db.sw_todolist.Category) {
            const modal = $('#editModal');
        ...
        static fetchCategories() {
            ui.assets.js.Proxy.Category_Search('', function (categories: Category[]) {
                Category.showCategories(categories);
            });
        }

        static searchCategories(searchTerm: string) {
            // Llamar al método getCategories de la clase Proxy para obtener las categorías que coincidan con el término de búsqueda
            ui.assets.js.Proxy.Category_Search(searchTerm, function (categories: Category[]) {
                Category.showCategories(categories);
            });
        }

        // Función para obtener los detalles de la categoría por su ID
        static getCategoryById(categoryId: string, callback?: (n: nMorph.core.db.sw_todolist.Category) => void) {

            ui.assets.js.Proxy.Category_GetByID(nMorph.framework.common.datatypes.UUID.fromString(categoryId),
                (n: nMorph.core.db.sw_todolist.Category) => {
                    if (callback) {
                        callback(n);
                    }
                });
