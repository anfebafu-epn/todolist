...
namespace ui {
    export class Person {
            ...
            head.load("config.js?v=" + new Date().getTime().toString(), // Hace que este archivo nunca haga caché, siempre lea la última configuración
                function () {
                    head.load(
                        ...
                        "person.css?v=" + current_version,
                        "person.js?v=" + current_version,
                        function () {
                            Person.Load();
                        });
                }
        ...
        static Load() {
            ...
            Person.LoggedUser = <nMorph.core.db.sw_todolist.Person>nMorph.core.proxy.tools.Packager.Unserialize(psession);
            ...
            $(document).on('click', '.editButton', function () {
                ...
                const person = Person.getPersonById(personId, (n: nMorph.core.db.sw_todolist.Person) => {
                    Person.openModal(n);
                });
            ...
            $('#btn_Save').click(function (event) {
                ...
                var personData: nMorph.core.db.sw_todolist.Person = new nMorph.core.db.sw_todolist.Person();
        ...
        static showPersons(persons: any[]) {
            const PersonsContainer = $('#usersContainer');
            PersonsContainer.empty();

            persons.forEach(person => {
                var p = <nMorph.core.db.sw_todolist.Person>person;
        ...
        static openModal(person: nMorph.core.db.sw_todolist.Person) {
            const modal = $('#editModal');
        ...
        static fetchPersons() {
            ui.assets.js.Proxy.Person_Search('', function (persons: Person[]) {
                Person.showPersons(persons);
            });
        }

        static searchPersons(searchTerm: string) {
            // Llamar al método getpersons de la clase Proxy para obtener las personas que coincidan con el término de búsqueda
            ui.assets.js.Proxy.Person_Search(searchTerm, function (persons: Person[]) {
                Person.showPersons(persons);
            });
        }

        // Función para obtener los detalles de la persona por su ID
        static getPersonById(personId: string, callback?: (n: nMorph.core.db.sw_todolist.Person) => void) {

            ui.assets.js.Proxy.Person_GetByID(nMorph.framework.common.datatypes.UUID.fromString(personId),
                (n: nMorph.core.db.sw_todolist.Person) => {
                    if (callback) {
                        callback(n);
                    }
                });
