...
class DropIndexClause implements IClause {
    ...
    public function new(CurrentDDL:DDL, IndexName:String,?TableName:String) {
    ...
    public function SCRIPT_PART(?args:String) {
        ...
        switch(CurrentDDL.getCurrentEngine()){
            case DatabaseEngineEnum.GENERIC:
                CurrentDDL.AddBlock("DROP INDEX \"" + IndexName + "\"");
            case DatabaseEngineEnum.MYSQL:
                CurrentDDL.AddBlock("DROP INDEX \"" + IndexName + "\" ON \""+TableName+"\"");
